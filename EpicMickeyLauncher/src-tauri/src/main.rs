...
fn main() {
    tauri::Builder::default().invoke_handler(tauri::generate_handler![playgame, download_mod, change_mod_status, delete_mod])
...
async fn change_mod_status(json: String, dumploc: String, gameid: String)
{
    ...
    if active {
        ...
        for file in files {
            ...
            if std::path::Path::new(&source_path).exists() && std::path::Path::new(&destination_path).exists() {
        ...
        for file in texturefiles {
            ...
            let mut path_final = remove_first(&file).expect("couldn't remove slash from string");
...
async fn download_mod(url: String, name: String, dumploc: String, gameid: String) -> String
{
  ...
  let mut file = File::create("C:/EMLStuff/Data/TEMP.zip").expect("failed to create temp file");
  ...
  println!("done downloading");
    ...
    if Path::new(&path_datafiles).exists()  {
        ...
        let path_backup_str = path_backup.clone().into_os_string().into_string().unwrap();

        fs::create_dir_all(&path_backup).expect("couldn't create backup folder");

        let mut path_search_clone = path_datafiles.clone();

        let mut path_datafiles_clone_str = path_datafiles.clone();

        let path_datafiles_str = path_datafiles_clone_str.into_os_string().into_string().unwrap();
        ...
        for entry in WalkDir::new(&path_search_clone) {
            ...
            if !p.path().is_file()
            {
               ...
               let mut p_str_final = remove_first(&p_str_shortened).expect("couldn't remove slash from string");
    ...
    if Path::new(&path_textures).exists() {
        ...
        let path_textures_str = &path_textures.clone().into_os_string().into_string().unwrap();
