...
<a style="margin: 0 auto; display:block; text-align: center" href="placetogetgames">Where do i get the games?</a>
...
<script>
  ...
  onMount(async () => {
    ...
    if(!jsonExists)
    {
     await writeTextFile({ path: appLocalDataDirPath + "games.json", contents: '[]' }); 
    }
