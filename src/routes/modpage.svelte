<script lang="ts">
    ...
    import {Game, type GameConfig, type Mod, ModState, Platform} from "./library/types";
    ...
    import {GameInstance, GetAllInstancesWithSameGame} from "./library/instance.svelte";
    ...
    async function Init() {
        ...
        compatibleInstances = GetAllInstancesWithSameGame(modInfo.Game.toUpperCase() as Game, modInfo.Platform.toUpperCase() as Platform)
        selectedInstance = activeInstance
        if (await selectedInstance.CheckMod(InternetModToUnifiedMod(modInfo)) == ModState.Incompatible) {
    ...
    async function CheckIfDownloaded() {
        let state = await selectedInstance.CheckMod(InternetModToUnifiedMod(modInfo))
    ...
    >
        ...
        ></span>
            ...
            <p>
                ...
                <select style="color:black;" bind:value={selectedInstance} oninput={() => SelectNewInstance()}
                >
                    {#each compatibleInstances as instance}
                        <option selected={instance === activeInstance}
                                value={instance}>{instance.gameConfig.uniqueID}</option>
                    {/each}
                </select>
        ...
        >{@html formattedDescription}</span>
            </div>
        </div>
    </div>
