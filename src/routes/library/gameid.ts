...
export async function IdentifyGame(idInfo: IdentifyInfo, quiet: boolean = false) {
...
export function GetGameWiiID(game: GameConfig) {
    ...
    let gameRelease = gameIdentity.releases.find((r: any) => r.platform.toUpperCase() == game.platform.toString())
    if (gameRelease.regions == null) {
        console.log(game.game + " is not a Wii game.")
        return ""
    }
    let gameRegion = gameRelease.regions.find((r: any) => Region[r.region.toUpperCase()] == game.region)
...
export function GetGameRelease(game: Game, platform: Platform): GameIdentityRelease {
    ...
    let gameRelease = gameIdentity.releases.find((r: any) => r.platform == platform)
