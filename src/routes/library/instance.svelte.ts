...
import {GetGameIdentity, GetGameRelease, GetGameWiiID, IdentifyGame, IdentifyISO, LocalModToUnifiedMod} from "./gameid";
import {exists} from "@tauri-apps/plugin-fs";
import {POST, serverLink} from "./networking";
import {RetrieveFileByAlias} from "./filealias";
import {path} from "@tauri-apps/api";
import {GetLoadedGameInstances, LoadConfig, RemoveTrackedGame} from "./config";

export function GetAllInstancesWithSameGame(game: Game, platform: Platform) {
    let gameInstances = GetLoadedGameInstances()
    return gameInstances.filter(r => r.gameConfig.game == game && r.gameConfig.platform == platform)
}
...
export class GameInstance {
    ...
    async Load(): Promise<boolean> {
        ...
        if (identifyGame.game == Game.EMR) {
            ...
            if (!ue4ssLockExists) {
                let confirmInstall = await confirm("Epic Mickey Rebrushed on EML requires UE4SS. Press Yes to automatically download & inject it. It will not replace any game files. If you cancel this prompt, mods that require it will not work, and you will be given this same prompt when the game is reinitialized.")
                if (confirmInstall) {
        ...
        if (!uidLockExists)
            await WriteFile(this.GetShortName(false).toLowerCase().replaceAll(" ", "_") + "_" + Date.now().toString(), this.path + "/eml-id.lock")
    ...
    GetShortName(fullName: boolean): string {
        return (fullName ? this.gameIdentity.name : this.gameConfig.game) + " (" +
            this.gameConfig.platform.toUpperCase() + (this.gameConfig.region !== Region.None ? (", " + this.gameConfig.region) : "") + ")"
    }
        ...
        if (this.gameConfig.platform.toUpperCase() == Platform.Wii) {
            ...
            invoke("get_os").then(async (_os) => {
                if (_os == "linux") {
                    ...
                    let gameRelease = GetGameRelease(this.gameConfig.game, this.gameConfig.platform)
                    invoke("playgame", {
                        dolphin: await path.join(this.gameConfig.path, gameRelease.identifier),
                        exe: "",
                        id: "",
                    }).then((res) => {
                        if (res == 1) {
    ...
    async AddMod(modData: UnifiedMod, url: string = ""): Promise<boolean> {
        ...
        let gameID =
            this.gameConfig.platform == Platform.Wii ? GetGameWiiID(this.gameConfig) : "";
    ...
    async RemoveMod(id: string) {
        ...
        let gameID = this.gameConfig.platform == Platform.PC ? "" : GetGameWiiID(this.gameConfig);
    ...
    async CheckMod(mod: UnifiedMod): Promise<ModState> {
        ...
        let localInstalledMod = this.mods.find((r: { modid: any }) => r.modid == mod.id);
    ...
    async ReadModList() {
        let path = await RetrieveFileByAlias("eml-mods-json", this.gameConfig.path, true)
