...
export async function LoadGamesConfig(): Promise<string[]> {
    if (!await exists(await RetrieveFileByAlias("eml-tracked-games", configPath))) {
        await SaveGamesConfig([])
        return []
    }

    return JSON.parse(await ReadFile(await RetrieveFileByAlias("eml-tracked-games", configPath)));
}
...
export async function LoadConfig(): Promise<ConfigFile> {
    ...
    return JSON.parse(await ReadFile(await RetrieveFileByAlias("eml-config", configPath)));
}
