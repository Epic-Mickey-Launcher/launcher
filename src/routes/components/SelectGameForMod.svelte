<script lang="ts">
  import { Game, Platform, GameConfig } from "../library/types";
...
{#if selectRegion}
  <div class="selectregion">
    <span style="position:relative;top:250px;">
      ...
      >
        {#each filteredGames as region}
          {#if region.installed != "installed"}
            <button style="width:100%;" on:click={() => cb(region)}
      ...
      <p>
        <button on:click={() => cb(null)} class="hyperlinkbutton">Back</button>
      </p></span
    >
  </div>
{/if}
