<script lang="ts">
  ...
  import { Game, Platform, PublisherMod } from "./library/types";
  ...
  async function ConfirmInput() {
    ...
    let modMetaString = await readTextFile(selectedPath + "/mod.json");
    ...
    await writeTextFile(selectedPath + "/mod.json", JSON.stringify(modMeta));
    await writeTextFile(selectedPath + "/description.md", description);
...
<div style="display:flex;height:70vw;">
  ...
  <dialog style="min-width: 300px;" open>
    ...
    <p>
      ...
      <input type="checkbox" bind:this={gitHubCheck} />
      <Question
